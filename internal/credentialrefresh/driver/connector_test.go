...
// testKVFormatter is a simple alternate formatter used to verify that the
// Connector honors a caller-supplied Formatter override.
func testKVFormatter(username, password, host string, port int, db string, opts map[string]string) string {
	return fmt.Sprintf("user=%s password=%s host=%s port=%d dbname=%s", username, password, host, port, db)
}
...
func TestConnectorCanUseAlternateFormatter(t *testing.T) {
	...
	dsn := testKVFormatter(username, password, host, port, db, nil)
	if d.ConnStr != dsn {
