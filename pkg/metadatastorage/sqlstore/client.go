...
import (
	...
	driverstores "github.com/in-toto/archivista/pkg/metadatastorage/sqlstore/driver-stores"
...
func NewEntClient(sqlBackend string, connectionString string, opts ...ClientOption) (*ent.Client, error) {
	...
	if strings.HasSuffix(upperSqlBackend, "_RDS_IAM") {
		...
		s = driverstores.NewStaticStore(user, password)
	...
	fmt.Printf("created connector for driver %s\n", driverName)
	fmt.Printf("original connection string: %s\n", connectionString)
	fmt.Printf("connection string: %s\n", dc.Formatter(user, "****", dc.Host, dc.Port, dc.DB, dc.Opts))
	...
	client := ent.NewClient(ent.Driver(sqlcommentDrv), ent.Debug(), ent.Log(log.Debug))
	return client, nil
}
