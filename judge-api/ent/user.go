...
func (u *User) String() string {
	...
	builder.WriteString(u.IdentityID)
	builder.WriteString(", ")
...
func (u *User) NamedTenant(name string) ([]*Tenant, error) {
	if u.Edges.namedTenant == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := u.Edges.namedTenant[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (u *User) appendNamedTenant(name string, edges ...*Tenant) {
	if u.Edges.namedTenant == nil {
		u.Edges.namedTenant = make(map[string][]*Tenant)
	}
	if len(edges) == 0 {
		u.Edges.namedTenant[name] = []*Tenant{}
	} else {
		u.Edges.namedTenant[name] = append(u.Edges.namedTenant[name], edges...)
	}
}
