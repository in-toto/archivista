...
func (pq *ProjectQuery) QueryTenant() *TenantQuery {
...
func (pq *ProjectQuery) WithTenant(opts ...func(*TenantQuery)) *ProjectQuery {
	query := (&TenantClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withTenant; query != nil {
		if err := pq.loadTenant(ctx, query, nodes, nil,
			func(n *Project, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Project, init func(*Project), assign func(*Project, *Tenant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
		if nodes[i].project_tenant == nil {
			continue
		}
		fk := *nodes[i].project_tenant
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_tenant" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *ProjectQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
		if nodes[i].project_created_by == nil {
			continue
		}
		fk := *nodes[i].project_created_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_created_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
