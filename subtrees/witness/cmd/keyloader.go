...
func loadSigners(ctx context.Context, so options.SignerOptions, signerProviders map[string]struct{}) ([]cryptoutil.Signer, error) {
	...
	for signerProvider := range signerProviders {
		...
		if err != nil {
			return signers, fmt.Errorf("failed to create %v signer provider: %w", signerProvider, err)
		}

		s, err := sp.Signer(ctx)
		if err != nil {
			return signers, fmt.Errorf("failed to create %v signer: %w", signerProvider, err)
		}
