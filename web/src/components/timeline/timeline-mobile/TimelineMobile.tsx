...
const TimelineMobile: React.FC<MobileTimelineProps> = ({ dsseArray }: MobileTimelineProps) => {
  ...
  const groupByCommit = dsseArray.reduce((acc: { [key: string]: any[] }, dsse) => {
    ...
    if (commit) {
      ...
      acc[commit]?.sort(
        (
          a: { signatures: { timestamps: { timestamp: string | number | Date }[] }[] },
          b: { signatures: { timestamps: { timestamp: string | number | Date }[] }[] }
        ) => {
  ...
  return (
    <Box sx={{ overflow: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {sortedCommits.map((commit, index) => {
        ...
        return (
          <StyledBox key={commit} sx={{ minWidth: 250, marginBottom: '16px' }}>
            ...
            {groupByCommit[commit].map(
              (
                dsse: { signatures: { timestamps: { timestamp: string | number | Date }[] }[]; id: any; statement: { attestationCollections: { name: any } } },
                index: number
              ) => {
