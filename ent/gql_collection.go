	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "metadata":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&MetadataClient{config: d.config}).Query()
			)
			if err := query.collectField(ctx, false, opCtx, field, path, mayAddCondition(satisfies, metadataImplementors)...); err != nil {
				return err
			}
			d.WithNamedMetadata(alias, func(wq *MetadataQuery) {
				*wq = *query
			})
		case "gitoidSha256":
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "envelope":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&DsseClient{config: m.config}).Query()
			)
			if err := query.collectField(ctx, false, opCtx, field, path, mayAddCondition(satisfies, dsseImplementors)...); err != nil {
				return err
			}
			m.WithNamedEnvelope(alias, func(wq *DsseQuery) {
				*wq = *query
			})
		case "key":
			if _, ok := fieldSeen[metadata.FieldKey]; !ok {
				selectedFields = append(selectedFields, metadata.FieldKey)
				fieldSeen[metadata.FieldKey] = struct{}{}
			}
		case "value":
			if _, ok := fieldSeen[metadata.FieldValue]; !ok {
				selectedFields = append(selectedFields, metadata.FieldValue)
				fieldSeen[metadata.FieldValue] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
