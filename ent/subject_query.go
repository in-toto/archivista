...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	...
	if query := sq.withStatement; query != nil {
		...
		for _, node := range nodes {
			...
			node.Edges.Statement = []*Statement{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(subject.StatementPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, sq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "statement": %w`, err)
		}
		query.Where(statement.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "statement" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Statement = append(nodes[i].Edges.Statement, n)
			}
		}
	}
