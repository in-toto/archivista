...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case timestamp.Table:
		...
		return query.Only(ctx)
	case vexdocument.Table:
		query := c.VexDocument.Query().
			Where(vexdocument.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, vexdocumentImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	case vexstatement.Table:
		query := c.VexStatement.Query().
			Where(vexstatement.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, vexstatementImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case vexdocument.Table:
		query := c.VexDocument.Query().
			Where(vexdocument.IDIn(ids...))
		query, err := query.CollectFields(ctx, vexdocumentImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
	...
	case vexstatement.Table:
		query := c.VexStatement.Query().
			Where(vexstatement.IDIn(ids...))
		query, err := query.CollectFields(ctx, vexstatementImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
