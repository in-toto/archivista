...
type MetadataQuery struct {
	...
	withEnvelope      *DsseQuery
	modifiers         []func(*sql.Selector)
	loadTotal         []func(context.Context, []*Metadata) error
	withNamedEnvelope map[string]*DsseQuery
...
// QueryEnvelope chains the current query on the "envelope" edge.
func (mq *MetadataQuery) QueryEnvelope() *DsseQuery {
	query := (&DsseClient{config: mq.config}).Query()
...
func (mq *MetadataQuery) Clone() *MetadataQuery {
	...
	return &MetadataQuery{
		...
		withEnvelope: mq.withEnvelope.Clone(),
...
// WithEnvelope tells the query-builder to eager-load the nodes that are connected to
// the "envelope" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MetadataQuery) WithEnvelope(opts ...func(*DsseQuery)) *MetadataQuery {
	query := (&DsseClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withEnvelope = query
	return mq
}
...
func (mq *MetadataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Metadata, error) {
	...
	if query := mq.withEnvelope; query != nil {
		if err := mq.loadEnvelope(ctx, query, nodes,
			func(n *Metadata) { n.Edges.Envelope = []*Dsse{} },
			func(n *Metadata, e *Dsse) { n.Edges.Envelope = append(n.Edges.Envelope, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range mq.withNamedEnvelope {
		if err := mq.loadEnvelope(ctx, query, nodes,
			func(n *Metadata) { n.appendNamedEnvelope(name) },
			func(n *Metadata, e *Dsse) { n.appendNamedEnvelope(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MetadataQuery) loadEnvelope(ctx context.Context, query *DsseQuery, nodes []*Metadata, init func(*Metadata), assign func(*Metadata, *Dsse)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Metadata)
	nids := make(map[int]map[*Metadata]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(metadata.EnvelopeTable)
		s.Join(joinT).On(s.C(dsse.FieldID), joinT.C(metadata.EnvelopePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(metadata.EnvelopePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(metadata.EnvelopePrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Dsse](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "envelope" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
// WithNamedEnvelope tells the query-builder to eager-load the nodes that are connected to the "envelope"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (mq *MetadataQuery) WithNamedEnvelope(name string, opts ...func(*DsseQuery)) *MetadataQuery {
	query := (&DsseClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if mq.withNamedEnvelope == nil {
		mq.withNamedEnvelope = make(map[string]*DsseQuery)
	}
	mq.withNamedEnvelope[name] = query
	return mq
}
